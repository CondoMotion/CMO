class SitesController < ApplicationController
  ...
  # GET /sites/1/edit
  def edit
    if current_user.memberships.find_by_site_id(current_user.sites.find(params[:id]).id).role.name == "site owner"
      @site = current_user.sites.find(params[:id])
    else
      redirect_to dashboard_url, notice: 'You do not have permission to edit that site'
    end
  end
